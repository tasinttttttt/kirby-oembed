<template>
  <k-field
    :input="_uid"
    v-bind="$props"
    :class="['k-oembed-field', status]"
  >
    <div
      class="k-input k-oembed-input"
      data-theme="field"
    >
      <input
        ref="input"
    ...
    <div class="k-oembed-container">
      ...
      >
        <div
          :id="embedId"
          class="k-oembed-embed"
          v-html="embedCode"
        />
        <div :class="['content', liststyle]">
          <div
            v-for="(val, propertyName) in sortedInfo"
            class="content-block"
          >
            <div class="title">
              {{ (!$t('oembed.embed.' + propertyName).includes('oembed') && $t('oembed.embed.' + propertyName) || propertyName) | capitalize }}
            </div>
            <div class="value">
              {{ val }}
            </div>
    ...
    getEmbed (url) {
      return new Promise((resolve, reject) => {
        if (url) {
          ...
          this.$api.post('oembed/preview', data)
            .then(response => {
              if (response && response.data) {
                if (response.data.xdebug_message || response.xdebug_message) {
                  throw Error(response.data.xdebug_message || response.xdebug_message)
                } else {
            ...
            .catch(error => {
              this.debug && console.log(error, this.$t('oembed.error.' + error.message))
              reject((error && error.message && this.$t('oembed.error.' + error.message)) || this.$t('oembed.error.generic'))
            })
        } else {
